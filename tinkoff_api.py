...
# Кеш названий инструментов
_instrument_cache = {}
# Кеш счетов
_accounts_cache = None

def money_to_float(money):
    """Превращает {'units': '189', 'nano': 0} в число 189.0"""
...
def get_accounts():
    """Получает список всех счетов (с кешем)"""
...
def get_instrument_name(figi):
    """Получает название инструмента по FIGI (с кешем)"""
...
def get_payments_for_account(account_id, days=30, from_date=None, to_date=None):
    """
    Возвращает готовый список выплат для отображения.
    Можно указать days ИЛИ from_date и to_date.
    """
...
def get_all_payments(account_ids=None, days=30, from_date=None, to_date=None):
    """
    Получает выплаты по всем счетам (или по списку счетов).
    Можно указать days ИЛИ from_date и to_date.
    """
...
def get_account_name(account_id):
    """Возвращает название счёта по ID"""
    accounts = get_accounts()
    for acc in accounts:
...
def get_portfolio(account_id):
    ...
    print(f"Ошибка получения портфеля: {response.status_code}")
    return None


def get_portfolio_positions(account_id):
    """Получает список позиций в портфеле (figi + количество + тип)"""
    response = requests.post(
...
def get_bond_coupons(figi, from_date, to_date):
    """Получает купоны по облигации за период"""
    response = requests.post(
...
def get_dividends(figi, from_date, to_date):
    """Получает дивиденды по акции за период"""
    response = requests.post(
...
def get_future_payments(account_id=None):
    """
    Собирает будущие выплаты (купоны + дивиденды).
    На месяц вперёд и до конца года.
    """
    today = datetime.now(timezone.utc)
...
def get_avg_coupon(account_id=None):
    """
    Считает среднюю купонную ставку по портфелю облигаций.
    Упрощённый метод: используем исторические выплаты за последние 6 месяцев.
    """
    today = datetime.now(timezone.utc)
...
if __name__ == "__main__":
    ...
    payments, total = get_all_payments(days=30)

    print()
    print("=" * 60)
    print(f"💰 ВСЕ ПОСТУПЛЕНИЯ ЗА 30 ДНЕЙ")
    print("=" * 60)
    print()
    ...
    for p in payments:
        print(f"📊 {p['name']}")
        print(f"   Тип: {p['type']}")
        print(f"   Сумма: {p['amount']:.2f} ₽")
        print(f"   Дата: {p['date']}")
        print(f"   Счёт ID: {p['account_id']}")
        print()
